...
class AIService:
    ...
    def review_code(self, code, language, lesson_title, lesson_description, lesson_task=""):
        ...
        message = self.client.messages.create(
            ...
            max_tokens=700,
            messages=[{"role": "user", "content": f"Lesson: {lesson_title}\nDescription: {lesson_description}{task_section}\nLanguage: {language}\nUser's code:\n{code}\n\nEvaluate this solution and respond with ONLY a valid JSON object, no markdown, no code blocks. The JSON must have exactly these fields:\n\"verdict\": \"CORRECT\" or \"INCORRECT\" — mark as CORRECT if the code logically solves the task described above. Be very lenient on anything cosmetic: capitalization, punctuation (commas, periods, exclamation marks), spelling, spacing, extra prints, slightly different variable names, and the exact wording or text of printed strings — these do not matter. Only mark INCORRECT if the code fundamentally does not implement what the task requires (wrong logic, missing the required function/feature, or completely off-topic).\n\"ai_solution\": your clean ideal solution for this specific task\n\"advice\": always give 1-2 sentences of helpful feedback regardless of verdict, focused ONLY on logic and concepts. NEVER mention or criticize capitalization, punctuation, spelling, spacing, exact wording, or any other cosmetic/formatting difference — these do NOT matter and must never appear in the advice. If correct, mention one thing done well or a small conceptual tip; do not nitpick formatting. If incorrect, explain clearly what logic is missing or wrong."}]
        )
